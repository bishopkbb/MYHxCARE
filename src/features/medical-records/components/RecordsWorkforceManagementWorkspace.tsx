...
const RecordsShiftDetailModal = dynamic(
  () => import('./RecordsShiftDetailModal').then((m) => m.RecordsShiftDetailModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
function exportRosterAsPDF(rows: RecordsShift[]) {
  const body = `
    <h1>Today's Medical Records Roster</h1>
  ...
  `;
  downloadPDF('medical-records-workforce-roster', body);
}

function exportRosterAsCSV(rows: RecordsShift[]) {
  downloadCSV('medical-records-workforce-roster', [
...
export function RecordsWorkforceManagementWorkspace() {
  ...
  const [editingShift, setEditingShift] = useState<RecordsShift | null>(null);
  const [viewingShift, setViewingShift] = useState<RecordsShift | null>(null);
  ...
  function handlePublishRoster() {
    toast.success('Roster published', 'Today’s roster has been published to all records staff.');
  }
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Manage medical records staff schedules, duty rosters, and shift coverage.
              </p>
            </div>
