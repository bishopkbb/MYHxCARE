...
// This screen's own display shape (`AdministrationShift`) mirrors
// BillingShift's pre-unification pattern — only the state source is the
// canonical `StaffShift` store; these two converters are the seam.
function toAdminView(s: StaffShift): AdministrationShift {
  const { homeModule: _homeModule, department: _department, ...rest } = s;
  return rest;
}
function fromAdminView(s: AdministrationShift): StaffShift {
  return { ...s, homeModule: 'administration' };
}

const CreateEditAdminShiftModal = dynamic(
  () => import('./CreateEditAdminShiftModal').then((m) => m.CreateEditAdminShiftModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const AdminShiftDetailModal = dynamic(
  () => import('./AdminShiftDetailModal').then((m) => m.AdminShiftDetailModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
const COLS = [
  { key: 'staff', label: 'Staff', width: 'w-[20%]', align: '' },
  { key: 'role', label: 'Role', width: 'w-[12%]', align: '' },
  { key: 'shift', label: 'Shift', width: 'w-28 shrink-0', align: '' },
  { key: 'time', label: 'Time', width: 'w-[14%]', align: '' },
  { key: 'team', label: 'Team', width: 'min-w-0 flex-1', align: '' },
...
function exportRosterAsPDF(rows: AdministrationShift[]) {
  const body = `
    <h1>Today's Administration Roster</h1>
    <p class="meta">${rows.length} shift${rows.length === 1 ? '' : 's'}</p>
    <table>
      <thead><tr><th>Staff</th><th>Role</th><th>Shift</th><th>Time</th><th>Team</th><th>Status</th></tr></thead>
      <tbody>
        ${rows
          .map(
            (r) =>
  ...
  `;
  downloadPDF('admin-workforce-roster', body);
}

function exportRosterAsCSV(rows: AdministrationShift[]) {
  downloadCSV('admin-workforce-roster', [
    ['Staff', 'Role', 'Shift', 'Time', 'Team', 'Status', 'Acknowledged'],
    ...rows.map((r) => [
      r.staffName,
      r.role,
...
export function AdministrationWorkforceManagementWorkspace() {
  const toast = useToast();
  const [pageState, setPageState] = useState<PageState>('loading');
  const roster = useStaffShifts()
    .filter((s) => s.homeModule === 'administration')
    .map(toAdminView);
  ...
  const [editingShift, setEditingShift] = useState<AdministrationShift | null>(null);
  const [viewingShift, setViewingShift] = useState<AdministrationShift | null>(null);
  ...
  // Derived from the live roster (not a static fixture) so acknowledging a
  // shift actually removes it from this list.
  ...
  function handleCreateShift(shift: AdministrationShift) {
    addShift(fromAdminView(shift));
    setCreateOpen(false);
    toast.success('Shift created', `${shift.staffName}'s shift has been added to the roster.`);
  }

  function handleUpdateShift(shift: AdministrationShift) {
    updateShiftInStore(fromAdminView(shift));
    setEditingShift(null);
    toast.success('Shift updated', `${shift.staffName}'s shift has been updated.`);
  }

  function handleCancelShift(shift: AdministrationShift) {
    cancelShiftInStore(shift.id);
    setOpenRowMenuId(null);
    toast.info('Shift cancelled', `${shift.staffName}'s shift has been marked as cancelled.`);
  }

  function handleDuplicateShift(shift: AdministrationShift) {
    duplicateShiftInStore(fromAdminView(shift));
    setOpenRowMenuId(null);
  ...
  function handlePublishRoster() {
    toast.success(
      'Roster published',
      'Today’s roster has been published to all Administration staff.',
    );
  }
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Manage Administration schedules, duty rosters, team coverage and shift operations.
              </p>
            </div>
                    ...
                    {paginated.map((shift) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="w-[12%] min-w-0 px-3 py-3">
