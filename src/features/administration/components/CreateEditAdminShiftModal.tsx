...
export function CreateEditAdminShiftModal({
  editingShift,
  onClose,
  onSave,
}: {
  editingShift?: AdministrationShift | undefined;
  onClose: () => void;
  onSave: (shift: AdministrationShift) => void;
}) {
  ...
  function handleSubmit() {
    setSubmitted(true);
    if (!isValid) {
      toast.error('Required', 'Please fill in staff name, role, and team.');
      return;
    }
    const shift: AdministrationShift = {
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth px-6 py-5">
          <div className="flex flex-col gap-4">
            <FormField
              label="Staff Name"
              htmlFor="adminshift-staff-name"
              required
              error={submitted && !staffName.trim() ? 'Required' : undefined}
            >
              <FormInput
                id="adminshift-staff-name"
                placeholder="e.g. Kelechi Obasi"
                value={staffName}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField label="Shift Type" htmlFor="adminshift-type">
                <FormSelect
                  id="adminshift-type"
                  value={shiftType}
              ...
              <FormField label="Status" htmlFor="adminshift-status">
                <FormSelect
                  id="adminshift-status"
                  value={status}
