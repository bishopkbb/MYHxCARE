'use client';

import { Check, X } from 'lucide-react';

import { formatCurrency } from '@/utils/currency';
import { formatDateTime, formatHumanDate } from '@/utils/datetime';
import type { ProcurementRequest } from '@/features/laboratory/__mocks__/procurementFixtures';

const FOCUS_RING =
  'focus-visible:ring-2 focus-visible:ring-[#00B4D8]/50 focus-visible:outline-none';

const STATUS_CFG: Record<string, { color: string; bg: string; border: string }> = {
  'Pending Approval': {
    color: '#B45309',
    bg: 'rgba(245,158,11,0.1)',
    border: 'rgba(245,158,11,0.35)',
  },
  Approved: { color: '#16A34A', bg: 'rgba(34,197,94,0.1)', border: 'rgba(34,197,94,0.35)' },
  'In Procurement': { color: '#2563EB', bg: 'rgba(37,99,235,0.1)', border: 'rgba(37,99,235,0.35)' },
  Received: { color: '#0D9488', bg: 'rgba(13,148,136,0.1)', border: 'rgba(13,148,136,0.35)' },
  Rejected: { color: '#DC2626', bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.35)' },
  Cancelled: { color: '#64748B', bg: 'rgba(100,116,139,0.1)', border: 'rgba(100,116,139,0.35)' },
};

const STEP_CFG: Record<string, { color: string }> = {
  Approved: { color: '#16A34A' },
  Rejected: { color: '#DC2626' },
  Skipped: { color: '#8A98A3' },
  Pending: { color: '#8A98A3' },
};

/** Read-only, richer detail view than the docked panel — full itemized
 * line items and the complete approval workflow with timestamps, not just
 * a category summary. Opens from the panel's external-link icon, the
 * "View Full Details" button, or a row's own eye icon. Lazy-loaded
 * (checklist §14). */
export function ProcurementRequestDetailModal({
  request,
  onClose,
}: {
  request: ProcurementRequest;
  onClose: () => void;
}) {
  const cfg = STATUS_CFG[request.status]!;
  const lineItemsTotal = request.lineItems.reduce(
    (sum, li) => sum + li.quantity * li.estimatedUnitCost,
    0,
  );

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center p-4"
      style={{ background: 'rgba(13,38,48,0.45)' }}
      onClick={(e) => {
        if (e.target === e.currentTarget) onClose();
      }}
    >
      <div
        className="flex w-full flex-col overflow-hidden bg-white"
        style={{ maxWidth: 760, maxHeight: 'calc(100vh - 48px)', borderRadius: 16 }}
      >
        <div
          className="flex shrink-0 items-start justify-between gap-3 px-6 py-5"
          style={{ borderBottom: '1px solid rgba(0,100,130,0.12)' }}
        >
          <div>
            <div className="flex items-center gap-2">
              <h2
                className="font-display font-semibold"
                style={{ fontSize: 20, lineHeight: '28px', color: '#0D2630' }}
              >
                {request.id}
              </h2>
              <span
                className="rounded-full px-2.5 py-0.5 font-sans font-medium"
                style={{
                  fontSize: 14,
                  color: cfg.color,
                  background: cfg.bg,
                  border: `1px solid ${cfg.border}`,
                }}
              >
                {request.status}
              </span>
            </div>
            <p className="mt-0.5" style={{ fontSize: 14, color: '#4A7080' }}>
              {request.department} · Requested by {request.requestedBy}
            </p>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label="Close"
            className={`flex size-11 shrink-0 items-center justify-center rounded-full transition-colors duration-150 hover:bg-[rgba(0,0,0,0.06)] ${FOCUS_RING}`}
          >
            <X style={{ width: 20, height: 20, color: '#4A7080' }} />
          </button>
        </div>

        <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth px-6 py-5">
          <div className="grid grid-cols-1 gap-x-6 gap-y-3 sm:grid-cols-2">
            {[
              ['Request Date', formatHumanDate(request.requestDate)],
              ['Priority', request.priority],
              ['Estimated Amount', formatCurrency(request.estimatedAmount)],
              [
                'Estimated Delivery',
                request.estimatedDeliveryDate
                  ? formatHumanDate(request.estimatedDeliveryDate)
                  : '—',
              ],
              ['Last Updated', formatDateTime(request.lastUpdated)],
              ['Origin', request.origin],
            ].map(([label, value]) => (
              <div key={label} className="flex items-center justify-between gap-2">
                <span style={{ fontSize: 14, color: '#8A98A3' }}>{label}</span>
                <span
                  className="text-right font-sans font-medium"
                  style={{ fontSize: 14, color: '#0D2630' }}
                >
                  {value}
                </span>
              </div>
            ))}
          </div>
          {request.notes && (
            <p className="mt-3" style={{ fontSize: 14, color: '#2F3A40' }}>
              {request.notes}
            </p>
          )}

          <div className="mt-6">
            <p className="font-sans font-semibold" style={{ fontSize: 16, color: '#0D2630' }}>
              Line Items
            </p>
            <div className="mt-2.5 flex flex-col gap-2">
              {request.lineItems.map((li) => (
                <div
                  key={li.id}
                  className="flex items-center justify-between gap-3 rounded-[10px] p-3"
                  style={{ background: '#F5FBFD', border: '1px solid rgba(0,100,130,0.08)' }}
                >
                  <div className="min-w-0 flex-1">
                    <p
                      className="truncate font-sans font-medium"
                      style={{ fontSize: 14, color: '#0D2630' }}
                    >
                      {li.name}
                    </p>
                    <p style={{ fontSize: 14, color: '#8A98A3' }}>
                      {li.category} · Qty {li.quantity}
                    </p>
                  </div>
                  <p
                    className="shrink-0 font-sans font-medium whitespace-nowrap"
                    style={{ fontSize: 14, color: '#0D2630' }}
                  >
                    {formatCurrency(li.quantity * li.estimatedUnitCost)}
                  </p>
                </div>
              ))}
            </div>
            <div className="mt-2.5 flex items-center justify-between gap-2 px-3">
              <span style={{ fontSize: 14, color: '#8A98A3' }}>
                Line items (indicative pricing)
              </span>
              <span className="font-sans font-medium" style={{ fontSize: 14, color: '#4A7080' }}>
                {formatCurrency(lineItemsTotal)}
              </span>
            </div>
          </div>

          <div className="mt-6">
            <p className="font-sans font-semibold" style={{ fontSize: 16, color: '#0D2630' }}>
              Approval Workflow
            </p>
            <div className="mt-2.5 flex flex-col gap-2.5">
              {request.approvalSteps.map((step) => (
                <div
                  key={step.role}
                  className="flex items-center justify-between gap-3 rounded-[10px] p-3"
                  style={{ background: '#F5FBFD', border: '1px solid rgba(0,100,130,0.08)' }}
                >
                  <div className="flex min-w-0 items-center gap-2.5">
                    <div
                      className="flex size-7 shrink-0 items-center justify-center rounded-full"
                      style={{
                        background:
                          step.status === 'Approved'
                            ? '#16A34A'
                            : step.status === 'Rejected'
                              ? '#DC2626'
                              : '#E2EDF1',
                      }}
                    >
                      {step.status === 'Approved' && (
                        <Check style={{ width: 13, height: 13, color: '#FFFFFF' }} />
                      )}
                      {step.status === 'Rejected' && (
                        <X style={{ width: 13, height: 13, color: '#FFFFFF' }} />
                      )}
                    </div>
                    <div className="min-w-0">
                      <p
                        className="truncate font-sans font-medium"
                        style={{ fontSize: 14, color: '#0D2630' }}
                      >
                        {step.role}
                      </p>
                      <p className="truncate" style={{ fontSize: 14, color: '#8A98A3' }}>
                        {step.actor ?? 'Not yet actioned'}
                      </p>
                    </div>
                  </div>
                  <div className="shrink-0 text-right">
                    <p
                      className="font-sans font-medium"
                      style={{ fontSize: 14, color: STEP_CFG[step.status]!.color }}
                    >
                      {step.status}
                    </p>
                    {step.at && (
                      <p style={{ fontSize: 14, color: '#8A98A3' }}>{formatDateTime(step.at)}</p>
                    )}
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>

        <div
          className="flex shrink-0 items-center justify-end px-6 py-4"
          style={{ borderTop: '1px solid rgba(0,100,130,0.12)' }}
        >
          <button
            type="button"
            onClick={onClose}
            className={`flex h-11 items-center rounded-[10px] px-5 font-sans font-medium transition-colors duration-150 hover:bg-[#F5FBFD] ${FOCUS_RING}`}
            style={{ fontSize: 14, color: '#0D2630', border: '1px solid rgba(0,100,130,0.2)' }}
          >
            Close
          </button>
        </div>
      </div>
    </div>
  );
}
