...
import type { HandoverLabScientist } from '@/features/laboratory/__mocks__/laboratoryShiftHandoverFixtures';

const FOCUS_RING =
  'focus-visible:ring-2 focus-visible:ring-[#00B4D8]/50 focus-visible:outline-none';

export function LaboratorySignHandoverModal({
  scientist,
  onConfirm,
  onClose,
}: {
  scientist: HandoverLabScientist;
  onConfirm: () => void;
  onClose: () => void;
}) {
        ...
        <div className="flex-1 px-6 py-5">
          ...
          >
            <div
              ...
              style={{ background: scientist.avatarBg, fontSize: 14 }}
            >
              {scientist.name
                .split(' ')
                .map((w) => w[0])
            ...
            <div className="min-w-0">
              ...
              <p style={{ fontSize: 14, color: '#8A98A3' }}>{scientist.staffId}</p>
            </div>
          </div>
          ...
          <label className="mt-4 flex cursor-pointer items-start gap-2.5">
            ...
            <span style={{ fontSize: 14, color: '#2F3A40', lineHeight: '22px' }}>
              I, {scientist.name}, confirm I have reviewed this shift handover and am accepting
              responsibility for the test pipeline and reagent stock listed above.
            </span>
          </label>
        </div>
