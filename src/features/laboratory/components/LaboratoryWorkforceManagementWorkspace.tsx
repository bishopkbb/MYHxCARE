...
// This screen's own display shape (`LaboratoryShift`) mirrors
// PharmacyShift's pre-unification pattern — only the state source is the
...
function fromLaboratoryView(s: LaboratoryShift): StaffShift {
  return { ...s, homeModule: 'laboratory' };
}

const CreateEditLaboratoryShiftModal = dynamic(
  () => import('./CreateEditLaboratoryShiftModal').then((m) => m.CreateEditLaboratoryShiftModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const LaboratoryShiftDetailModal = dynamic(
  () => import('./LaboratoryShiftDetailModal').then((m) => m.LaboratoryShiftDetailModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
const COLS = [
  ...
  { key: 'department', label: 'Department', width: 'min-w-0 flex-1', align: '' },
...
function exportRosterAsPDF(rows: LaboratoryShift[]) {
  const body = `
    <h1>Today's Laboratory Roster</h1>
    <p class="meta">${rows.length} shift${rows.length === 1 ? '' : 's'}</p>
    <table>
      <thead><tr><th>Staff</th><th>Role</th><th>Shift</th><th>Time</th><th>Department</th><th>Status</th></tr></thead>
      <tbody>
        ${rows
          .map(
            (r) =>
  ...
  `;
  downloadPDF('laboratory-workforce-roster', body);
}

function exportRosterAsCSV(rows: LaboratoryShift[]) {
  downloadCSV('laboratory-workforce-roster', [
    ['Staff', 'Role', 'Shift', 'Time', 'Department', 'Status', 'Acknowledged'],
    ...rows.map((r) => [
      r.staffName,
      r.role,
...
export function LaboratoryWorkforceManagementWorkspace() {
  ...
  const [editingShift, setEditingShift] = useState<LaboratoryShift | null>(null);
  const [viewingShift, setViewingShift] = useState<LaboratoryShift | null>(null);
  ...
  function handleCreateShift(shift: LaboratoryShift) {
    addShift(fromLaboratoryView(shift));
    setCreateOpen(false);
    toast.success('Shift created', `${shift.staffName}'s shift has been added to the roster.`);
  }

  function handleUpdateShift(shift: LaboratoryShift) {
    updateShiftInStore(fromLaboratoryView(shift));
    setEditingShift(null);
    toast.success('Shift updated', `${shift.staffName}'s shift has been updated.`);
  }

  function handleCancelShift(shift: LaboratoryShift) {
    cancelShiftInStore(shift.id);
    setOpenRowMenuId(null);
    toast.info('Shift cancelled', `${shift.staffName}'s shift has been marked as cancelled.`);
  }

  function handleDuplicateShift(shift: LaboratoryShift) {
    duplicateShiftInStore(fromLaboratoryView(shift));
    setOpenRowMenuId(null);
  ...
  function handlePublishRoster() {
    toast.success('Roster published', 'Today’s roster has been published to all laboratory staff.');
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Manage laboratory schedules, duty rosters, department coverage and shift operations.
              </p>
            </div>
      ...
      {createOpen && (
        <CreateEditLaboratoryShiftModal
