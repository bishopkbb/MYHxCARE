...
export function CreateEditLaboratoryShiftModal({
  editingShift,
  onClose,
  onSave,
}: {
  editingShift?: LaboratoryShift | undefined;
  onClose: () => void;
  onSave: (shift: LaboratoryShift) => void;
}) {
  ...
  function handleSubmit() {
    setSubmitted(true);
    if (!isValid) {
      toast.error('Required', 'Please fill in staff name, role, and department.');
      return;
    }
    const shift: LaboratoryShift = {
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth px-6 py-5">
          <div className="flex flex-col gap-4">
            <FormField
              label="Staff Name"
              htmlFor="labshift-staff-name"
              required
              error={submitted && !staffName.trim() ? 'Required' : undefined}
            >
              <FormInput
                id="labshift-staff-name"
                placeholder="e.g. Adaora Ugwu"
                value={staffName}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField label="Shift Type" htmlFor="labshift-type">
                <FormSelect
                  id="labshift-type"
                  value={shiftType}
              ...
              <FormField label="Status" htmlFor="labshift-status">
                <FormSelect
                  id="labshift-status"
                  value={status}
