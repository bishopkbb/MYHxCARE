...
// ── Order derivation ─────────────────────────────────────────────────────────
// An "Order" is not a stored entity — it's every LabResult row that shares the
// same patient and the same orderedAt timestamp, which is real and true by
// construction: addLabOrder() (labResultStore.ts) stamps every test submitted
// in one doctor request with one shared orderedAt. Computed fresh on every
// render from the live store; never persisted.
...
function worstPriority(tests: LabResult[]): LabResultPriority {
...
/** Presentation-only derived label, never persisted — same convention as the
 * Dashboard's own `deriveLabNo()`, distinct `ORD` prefix so an order-level ID
 * is never confused with a per-test lab number. */
function deriveOrderId(groupKey: string, orderedAt: string): string {
...
function buildOrder(groupKey: string, tests: LabResult[]): LabOrder {
  const first = tests[0]!;
  ...
  const completion =
    status === 'Completed' && maxResultAt
      ? {
          ...
          tatMs: new Date(maxResultAt).getTime() - new Date(first.orderedAt).getTime(),
        }
      : {};
  return {
...
export function LaboratoryOrdersWorkspace() {
  ...
  const orders = useMemo<LabOrder[]>(() => {
    ...
    return Array.from(groups.entries())
      .map(([groupKey, tests]) => buildOrder(groupKey, tests))
