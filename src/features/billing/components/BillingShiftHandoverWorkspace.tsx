...
import {
  ...
  OUTSTANDING_TASKS,
  type HandoverTask,
} from '@/features/billing/__mocks__/billingShiftHandoverFixtures';
import {
  buildAllInvoices,
  buildAllPayments,
  buildAllRefunds,
...
const BillingHandoverHistoryModal = dynamic(
  () => import('./BillingHandoverHistoryModal').then((m) => m.BillingHandoverHistoryModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const BillingHandoverTasksModal = dynamic(
  () => import('./BillingHandoverTasksModal').then((m) => m.BillingHandoverTasksModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const BillingSignHandoverModal = dynamic(
  () => import('./BillingSignHandoverModal').then((m) => m.BillingSignHandoverModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
export function BillingShiftHandoverWorkspace() {
  ...
  // ── Live billing backlog data ────────────────────────────────────────────
  const [now] = useState(() => Date.now());
  const [allInvoices] = useState(() => buildAllInvoices());
  ...
  const paymentsPostedToday = useMemo(
    () => allPayments.filter((p) => isToday(p.date)),
    [allPayments],
  );
  const unreconciledPayments = useMemo(
    () => allPayments.filter((p) => !p.reconciled),
    [allPayments],
  );
    ...
    () =>
      unpaidInvoices
        .map((inv) => ({
          ...
          daysOverdue: Math.max(0, Math.floor((now - new Date(inv.dueDate).getTime()) / DAY_MS)),
        }))
  ...
  const refundPreview = pendingRefunds.slice(0, 5);
  const overduePreview = overdueInvoices.slice(0, 5);
  const unreconciledPreview = unreconciledPayments.slice(0, 5);
  ...
  function handleCompleteHandover() {
    ...
    toast.success(
      'Handover completed',
      'Responsibility for the Finance Department has been transferred.',
    );
  }
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      <main className="flex-1 overflow-y-auto scroll-smooth" style={{ background: '#F5FBFD' }}>
        <div className="mx-auto max-w-[1200px] px-4 py-4 sm:px-6 sm:py-5">
          ...
          <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Ensure safe and effective continuity of the billing backlog and reconciliation
                queue.
              </p>
            </div>
            <button
          ...
          {completed && (
            ...
            >
              ...
              <p style={{ fontSize: 14, color: '#166534' }}>
                ...
                . Responsibility for the Finance Department has been formally transferred.
              </p>
            </div>
          )}
          ...
          {/* ── Billing Summary ────────────────────────────────────────────── */}
          <div className="mt-4">
            <SectionCard title="Billing Summary" icon={Wallet} iconColor="#00B4D8">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <StatTile
                  ...
                  value={paymentsPostedToday.length}
                />
                <StatTile
                  icon={AlertTriangle}
                  color="#EF4444"
                  ...
                  value={unreconciledPayments.length}
                />
              </div>
              ...
              <div className="mt-4">
                <ScrollableTable minWidth={760}>
                  ...
                  >
                    ...
                    <span
                      className="w-28 shrink-0 py-2.5 pr-3 text-right font-sans font-bold tracking-wider uppercase"
                      style={{ fontSize: 14, color: '#4A7080' }}
                    >
                      Amount
                    </span>
                  ...
                  {refundPreview.length === 0 ? (
                    <EmptyRow label="No refunds pending approval." />
                  ) : (
                    refundPreview.map((refund, idx) => (
                      <div
                        key={refund.id}
                        className="flex items-center"
                        style={{
                          borderBottom:
                            idx === refundPreview.length - 1
                              ? undefined
                              : '1px solid rgba(0,100,130,0.08)',
                        }}
                      >
                        <div className="w-36 shrink-0 py-3 pr-2 pl-3">
                          <Tooltip content={refund.refundNumber}>
                        ...
                        <div className="w-36 shrink-0 py-3 pr-2">
                          <Tooltip content={refund.patientName}>
                            <p
                        ...
                        <div className="min-w-0 flex-1 py-3 pr-2">
                          <Tooltip content={refund.reason}>
                            <p className="truncate" style={{ fontSize: 14, color: '#4A7080' }}>
                              {refund.reason}
                            </p>
                          </Tooltip>
                        </div>
                        <div className="w-28 shrink-0 py-3 pr-3 text-right">
                          <p style={{ fontSize: 14, color: '#0D2630' }}>
                            {formatCurrencyWhole(refund.amount)}
                          </p>
                        </div>
                      </div>
                    ))
          ...
          {/* ── Outstanding Tasks + Overdue Invoices ────────────────────── */}
          <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
            ...
            <SectionCard title="Overdue Invoices" icon={AlertTriangle} iconColor="#EF4444">
              <ScrollableTable minWidth={560}>
                ...
                >
                  ...
                  <span
                    className="w-36 shrink-0 py-2.5 pr-2 text-right font-sans font-bold tracking-wider uppercase"
                    style={{ fontSize: 14, color: '#4A7080' }}
                  >
                    Balance
                  </span>
                  <span
                    className="min-w-0 flex-1 py-2.5 pr-3 text-right font-sans font-bold tracking-wider uppercase"
                    style={{ fontSize: 14, color: '#4A7080' }}
                  >
                    Days Overdue
                  </span>
                ...
                {overduePreview.length === 0 ? (
                  <EmptyRow label="No overdue invoices right now." />
                ) : (
                  overduePreview.map((inv, idx) => (
                    <div
                      key={inv.id}
                      className="flex items-center"
                      style={{
                        borderBottom:
                          idx === overduePreview.length - 1
                            ? undefined
                            : '1px solid rgba(0,100,130,0.08)',
                      }}
                    >
                      ...
                      <div className="min-w-0 flex-1 py-3 pr-3 text-right">
                        <p
                          className="font-sans font-medium"
          ...
          <div className="mt-4">
            <SectionCard title="Unreconciled Payments" icon={RefreshCcw} iconColor="#D97706">
              <ScrollableTable minWidth={560}>
                <div
                  className={`flex ${TABLE_HEADER_STICKY_CLASS}`}
                  style={{
                ...
                {unreconciledPreview.length === 0 ? (
                  <EmptyRow label="No unreconciled payments right now." />
                ) : (
                  unreconciledPreview.map((p, idx) => (
                    <div
                      key={p.id}
                      className="flex items-center"
                      style={{
                        borderBottom:
                          idx === unreconciledPreview.length - 1
                            ? undefined
                            : '1px solid rgba(0,100,130,0.08)',
                      }}
                    >
                      <div className="min-w-0 flex-1 py-3 pr-2 pl-3">
                        <Tooltip content={p.paymentNumber}>
                          <p
                            className="truncate font-sans font-medium"
                            style={{ fontSize: 14, color: '#0D2630' }}
                          >
      ...
      {historyOpen && <BillingHandoverHistoryModal onClose={() => setHistoryOpen(false)} />}
      {tasksModalOpen && (
        <BillingHandoverTasksModal
          tasks={tasks}
          onToggle={toggleTask}
