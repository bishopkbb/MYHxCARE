...
// This screen's own display shape (`BillingShift`) mirrors
// LaboratoryShift's pre-unification pattern — only the state source is the
// canonical `StaffShift` store; these two converters are the seam.
function toBillingView(s: StaffShift): BillingShift {
  const { homeModule: _homeModule, department: _department, ...rest } = s;
  return rest;
}
function fromBillingView(s: BillingShift): StaffShift {
  return { ...s, homeModule: 'billing' };
}

const CreateEditBillingShiftModal = dynamic(
  () => import('./CreateEditBillingShiftModal').then((m) => m.CreateEditBillingShiftModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const BillingShiftDetailModal = dynamic(
  () => import('./BillingShiftDetailModal').then((m) => m.BillingShiftDetailModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
function exportRosterAsPDF(rows: BillingShift[]) {
  const body = `
    <h1>Today's Finance Department Roster</h1>
  ...
  `;
  downloadPDF('billing-workforce-roster', body);
}

function exportRosterAsCSV(rows: BillingShift[]) {
  downloadCSV('billing-workforce-roster', [
...
export function BillingWorkforceManagementWorkspace() {
  const toast = useToast();
  const [pageState, setPageState] = useState<PageState>('loading');
  const roster = useStaffShifts()
    .filter((s) => s.homeModule === 'billing')
    .map(toBillingView);
  ...
  const [editingShift, setEditingShift] = useState<BillingShift | null>(null);
  const [viewingShift, setViewingShift] = useState<BillingShift | null>(null);
  ...
  function handleCreateShift(shift: BillingShift) {
    addShift(fromBillingView(shift));
    setCreateOpen(false);
    toast.success('Shift created', `${shift.staffName}'s shift has been added to the roster.`);
  }

  function handleUpdateShift(shift: BillingShift) {
    updateShiftInStore(fromBillingView(shift));
    setEditingShift(null);
    toast.success('Shift updated', `${shift.staffName}'s shift has been updated.`);
  }

  function handleCancelShift(shift: BillingShift) {
    cancelShiftInStore(shift.id);
    setOpenRowMenuId(null);
    toast.info('Shift cancelled', `${shift.staffName}'s shift has been marked as cancelled.`);
  }

  function handleDuplicateShift(shift: BillingShift) {
    duplicateShiftInStore(fromBillingView(shift));
    setOpenRowMenuId(null);
  ...
  function handlePublishRoster() {
    toast.success(
      'Roster published',
      'Today’s roster has been published to all Finance Department staff.',
    );
  }
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Manage Finance Department schedules, duty rosters, team coverage and shift
                operations.
              </p>
            </div>
