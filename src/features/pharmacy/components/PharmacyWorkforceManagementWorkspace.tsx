...
// This screen's own display shape (`PharmacyShift`) mirrors NurseShift's
// pre-unification pattern — only the state source is the canonical
// `StaffShift` store; these two converters are the seam.
function toPharmacyView(s: StaffShift): PharmacyShift {
  const { homeModule: _homeModule, department: _department, ...rest } = s;
  return rest;
}
function fromPharmacyView(s: PharmacyShift): StaffShift {
  return { ...s, homeModule: 'pharmacy' };
}

const CreateEditPharmacyShiftModal = dynamic(
  () => import('./CreateEditPharmacyShiftModal').then((m) => m.CreateEditPharmacyShiftModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const PharmacyShiftDetailModal = dynamic(
  () => import('./PharmacyShiftDetailModal').then((m) => m.PharmacyShiftDetailModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
const COLS = [
  ...
  { key: 'location', label: 'Location', width: 'min-w-0 flex-1', align: '' },
...
function exportRosterAsPDF(rows: PharmacyShift[]) {
  const body = `
    <h1>Today's Pharmacy Roster</h1>
    <p class="meta">${rows.length} shift${rows.length === 1 ? '' : 's'}</p>
    <table>
      <thead><tr><th>Staff</th><th>Role</th><th>Shift</th><th>Time</th><th>Location</th><th>Status</th></tr></thead>
      <tbody>
        ${rows
          .map(
            (r) =>
  ...
  `;
  downloadPDF('pharmacy-workforce-roster', body);
}

function exportRosterAsCSV(rows: PharmacyShift[]) {
  downloadCSV('pharmacy-workforce-roster', [
    ['Staff', 'Role', 'Shift', 'Time', 'Location', 'Status', 'Acknowledged'],
    ...rows.map((r) => [
      r.staffName,
      r.role,
...
export function PharmacyWorkforceManagementWorkspace() {
  const toast = useToast();
  const [pageState, setPageState] = useState<PageState>('loading');
  const roster = useStaffShifts()
    .filter((s) => s.homeModule === 'pharmacy')
    .map(toPharmacyView);
  ...
  const [editingShift, setEditingShift] = useState<PharmacyShift | null>(null);
  const [viewingShift, setViewingShift] = useState<PharmacyShift | null>(null);
  ...
  function handleCreateShift(shift: PharmacyShift) {
    addShift(fromPharmacyView(shift));
    setCreateOpen(false);
    toast.success('Shift created', `${shift.staffName}'s shift has been added to the roster.`);
  }

  function handleUpdateShift(shift: PharmacyShift) {
    updateShiftInStore(fromPharmacyView(shift));
    setEditingShift(null);
    toast.success('Shift updated', `${shift.staffName}'s shift has been updated.`);
  }

  function handleCancelShift(shift: PharmacyShift) {
    cancelShiftInStore(shift.id);
    setOpenRowMenuId(null);
    toast.info('Shift cancelled', `${shift.staffName}'s shift has been marked as cancelled.`);
  }

  function handleDuplicateShift(shift: PharmacyShift) {
    duplicateShiftInStore(fromPharmacyView(shift));
    setOpenRowMenuId(null);
  ...
  function handlePublishRoster() {
    toast.success('Roster published', 'Today’s roster has been published to all pharmacy staff.');
  }
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Manage pharmacy schedules, duty rosters, campus coverage and shift operations.
              </p>
            </div>
