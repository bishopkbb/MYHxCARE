...
import type { HandoverPharmacist } from '@/features/pharmacy/__mocks__/pharmacyShiftHandoverFixtures';

const FOCUS_RING =
  'focus-visible:ring-2 focus-visible:ring-[#00B4D8]/50 focus-visible:outline-none';

export function PharmacySignHandoverModal({
  pharmacist,
  onConfirm,
  onClose,
}: {
  pharmacist: HandoverPharmacist;
  onConfirm: () => void;
  onClose: () => void;
}) {
        ...
        <div className="flex-1 px-6 py-5">
          ...
          >
            <div
              ...
              style={{ background: pharmacist.avatarBg, fontSize: 14 }}
            >
              {pharmacist.name
                .split(' ')
                .map((w) => w[0])
            ...
            <div className="min-w-0">
              ...
              <p style={{ fontSize: 14, color: '#8A98A3' }}>{pharmacist.staffId}</p>
            </div>
          </div>
          ...
          <label className="mt-4 flex cursor-pointer items-start gap-2.5">
            ...
            <span style={{ fontSize: 14, color: '#2F3A40', lineHeight: '22px' }}>
              I, {pharmacist.name}, confirm I have reviewed this shift handover and am accepting
              responsibility for the prescription queue and stock listed above.
            </span>
          </label>
        </div>
