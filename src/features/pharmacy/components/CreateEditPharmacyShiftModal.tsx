...
export function CreateEditPharmacyShiftModal({
  editingShift,
  onClose,
  onSave,
}: {
  editingShift?: PharmacyShift | undefined;
  onClose: () => void;
  onSave: (shift: PharmacyShift) => void;
}) {
  ...
  function handleSubmit() {
    setSubmitted(true);
    if (!isValid) {
      toast.error('Required', 'Please fill in staff name, role, and location.');
      return;
    }
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth px-6 py-5">
          <div className="flex flex-col gap-4">
            <FormField
              label="Staff Name"
              htmlFor="phshift-staff-name"
              required
              error={submitted && !staffName.trim() ? 'Required' : undefined}
            >
              <FormInput
                id="phshift-staff-name"
                placeholder="e.g. Pharm. Adaeze"
                value={staffName}
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <FormField label="Shift Type" htmlFor="phshift-type">
                <FormSelect
                  id="phshift-type"
                  value={shiftType}
              ...
              <FormField label="Status" htmlFor="phshift-status">
                <FormSelect
                  id="phshift-status"
                  value={status}
