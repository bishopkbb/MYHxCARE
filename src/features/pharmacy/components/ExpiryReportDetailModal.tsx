...
import { BUCKET_CFG } from '@/features/pharmacy/__mocks__/expiryReportFixtures';
...
/** Read-only snapshot view of one real batch's expiry status — lazy-loaded
 * (checklist §14). "Go to Expiry Management" is the genuine link for anyone
 * who wants to actually mark it returned or transfer it; this report stays
 * read-only. */
export function ExpiryReportDetailModal({
  row,
  bucket,
  daysLeft,
  onClose,
  ...
  const bucketCfg = BUCKET_CFG[bucket];
      ...
      >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto scroll-smooth px-6 py-3">
          <div className="flex items-center justify-between gap-3 rounded-[10px] bg-[#F5FBFD] px-3.5 py-2.5">
            ...
            >
              {bucket === 'Expired' ? 'Expired' : `${daysLeft} days left`}
            </span>
          </div>
          ...
          <div className="mt-1 divide-y" style={{ borderColor: 'rgba(0,100,130,0.08)' }}>
            ...
            <DetailRow label="Value at Risk" value={formatCurrency(row.stockQty * row.unitPrice)} />
          </div>
        </div>
