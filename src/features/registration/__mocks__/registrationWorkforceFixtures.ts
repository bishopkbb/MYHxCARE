/**
 * Mock fixtures for the Registration Workforce Management screen (supervisor-only).
...
const STATION_ROLE: Record<string, string[]> = {
  'Front Desk': ['Registration Officer', 'Senior Registration Officer'],
  'Insurance Desk': ['Insurance Desk Officer', 'Registration Officer'],
  'Emergency Registration': ['Senior Registration Officer', 'Registration Officer'],
  'Appointment Desk': ['Appointment Scheduler', 'Registration Officer'],
  'Records Intake': ['Front Desk Officer', 'Registration Officer'],
};
...
export const WORKFORCE_STATS = {
  onDuty: MOCK_REGISTRATION_ROSTER.filter((s) => s.status === 'ON_DUTY').length,
  todaysShifts: MOCK_REGISTRATION_ROSTER.length,
  onCall: MOCK_REGISTRATION_ROSTER.filter((s) => s.status === 'ON_CALL').length,
  pendingAck: MOCK_REGISTRATION_ROSTER.filter((s) => !s.acknowledged).length,
  coveragePercent: 90,
  pendingChanges: 2,
};
...
export const PENDING_ACKNOWLEDGEMENTS: PendingAcknowledgement[] = MOCK_REGISTRATION_ROSTER.filter(
  (s) => !s.acknowledged,
)
  .slice(0, 5)
  .map((s) => ({
...
export function nextShiftId(): string {
  return `rgs-${String(MOCK_REGISTRATION_ROSTER.length + 1).padStart(3, '0')}`;
}
