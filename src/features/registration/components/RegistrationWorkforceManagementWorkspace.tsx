...
const CreateEditRegistrationShiftModal = dynamic(
  () =>
    import('./CreateEditRegistrationShiftModal').then((m) => m.CreateEditRegistrationShiftModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
const RegistrationShiftDetailModal = dynamic(
  () => import('./RegistrationShiftDetailModal').then((m) => m.RegistrationShiftDetailModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
const COLS = [
  ...
  { key: 'station', label: 'Station', width: 'min-w-0 flex-1', align: '' },
...
function exportRosterAsPDF(rows: RegistrationShift[]) {
  const body = `
    <h1>Today's Registration Roster</h1>
    <p class="meta">${rows.length} shift${rows.length === 1 ? '' : 's'}</p>
    <table>
      <thead><tr><th>Staff</th><th>Role</th><th>Shift</th><th>Time</th><th>Station</th><th>Status</th></tr></thead>
      <tbody>
        ${rows
          .map(
            (r) =>
  ...
  `;
  downloadPDF('registration-workforce-roster', body);
}

function exportRosterAsCSV(rows: RegistrationShift[]) {
  downloadCSV('registration-workforce-roster', [
    ['Staff', 'Role', 'Shift', 'Time', 'Station', 'Status', 'Acknowledged'],
    ...rows.map((r) => [
      r.staffName,
      r.role,
...
export function RegistrationWorkforceManagementWorkspace() {
  ...
  const [editingShift, setEditingShift] = useState<RegistrationShift | null>(null);
  const [viewingShift, setViewingShift] = useState<RegistrationShift | null>(null);
  ...
  function handlePublishRoster() {
    toast.success(
      'Roster published',
      'Today’s roster has been published to all registration staff.',
    );
  }
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Manage registration desk schedules, duty rosters, and shift coverage.
              </p>
            </div>
      ...
      {createOpen && (
        <CreateEditRegistrationShiftModal
