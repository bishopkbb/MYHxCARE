...
const AdmissionChecklistModal = dynamic(
  () => import('./AdmissionChecklistModal').then((m) => m.AdmissionChecklistModal),
  { ssr: false, loading: () => <ModalLoadingFallback /> },
);
...
type TabKey = 'current' | 'pending' | 'completed' | 'all';
...
const TYPE_FILTER_OPTIONS: { value: string; label: string }[] = [
  { value: 'All', label: 'All Admission Types' },
...
function RowMenu({
  ...
  const canAdvance = admission.status !== 'Completed' && admission.status !== 'Cancelled';
  const canCancel = admission.status !== 'Completed' && admission.status !== 'Cancelled';
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        type="button"
        onClick={onToggle}
        aria-label={`More actions for ${admission.patientName}`}
...
export function AdmissionsWorkspace() {
  ...
  function handleRefresh() {
    ...
    setTimeout(() => {
      setPageState('loaded');
      toast.success('Admissions refreshed', 'The admissions list is up to date.');
    }, 700);
  }

  const wardOptions = useMemo(() => {
    const wards = Array.from(new Set(admissions.map((a) => a.ward)));
  ...
  const overview = useMemo(() => {
    return {
      ...
      scheduled: admissions.filter((a) => a.status === 'Scheduled').length,
      cancelled: admissions.filter((a) => a.status === 'Cancelled').length,
    };
  }, [admissions]);

  const stepProgress = useMemo(() => {
    return ADMISSION_STEPS.map((s) => ({
      ...s,
      count: admissions.filter((a) => a.status !== 'Cancelled' && a.currentStep >= s.step).length,
    }));
  }, [admissions]);
  ...
  const tabFiltered = useMemo(() => {
    return admissions.filter((a) => {
      if (activeTab === 'current') return a.status === 'In Progress' || a.status === 'Scheduled';
  ...
  const filtered = useMemo(() => {
    return tabFiltered.filter((a) => {
      ...
      if (typeFilter !== 'All' && a.admissionType !== typeFilter) return false;
      if (stepFilter !== null && a.currentStep !== stepFilter) return false;
      if (search.trim()) {
        const q = search.trim().toLowerCase();
        if (!(a.patientName.toLowerCase().includes(q) || a.mrn.toLowerCase().includes(q))) {
          return false;
        }
      }
      return true;
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
          ...
          <div className="mt-2 flex flex-wrap items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Track and manage newly admitted patients through the admission workflow.
              </p>
            </div>
              ...
              <div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-start">
                ...
                >
                  ...
                  >
                    {(
                      [
                        { key: 'current', label: 'Current Admissions', count: null },
                        {
                          key: 'pending',
                          label: 'Pending Admissions',
                          count: admissions.filter((a) => a.status === 'Pending').length,
                        },
                        {
                          key: 'completed',
                          label: 'Completed Today',
                          count: admissions.filter((a) => a.status === 'Completed').length,
                        },
                        { key: 'all', label: 'All Admissions', count: null },
                    ...
                    <div className="min-w-[1020px]">
                      ...
                      {pageState === 'loaded' &&
                        pageRows.map((a) => {
                          ...
                          const step = stepInfo(a.currentStep);
                          const StepIcon = a.currentStep > 0 ? STEP_ICON[a.currentStep] : null;
                              ...
                              <div className="min-w-[170px] flex-1 py-3 pr-1.5 pl-3">
                                <div className="flex items-center gap-2">
                                  <div
                                    ...
                                    style={{
                                      background: avatarColorFor(a.patientName),
                                      fontSize: 14,
                                    }}
                                  >
                                ...
                                {step && StepIcon ? (
                                  <div className="flex items-center gap-1.5">
                                    <div
                                      ...
                                      style={{ background: `${STEP_COLOR[a.currentStep]}1A` }}
                                    >
                                      <StepIcon
                                        style={{
                                          width: 13,
                                          height: 13,
                                          color: STEP_COLOR[a.currentStep],
                                        }}
                                      />
                                    </div>
                                    <p
                              ...
                              >
                                <PermissionGate permission={PERMISSIONS.ENCOUNTERS_WRITE}>
                                  <RowMenu
                                    admission={a}
                                    open={isMenuOpen}
                                    onToggle={() => setOpenMenuId(isMenuOpen ? null : a.id)}
                                    onViewPatient={() => {
                                      setOpenMenuId(null);
                                      viewPatient(a);
                                    }}
                                    onAdvance={() => advanceWorkflow(a)}
                                    onCancel={() => cancelAdmission(a)}
                                  />
                                </PermissionGate>
                              </div>
                            </div>
                ...
                <div className="flex w-full shrink-0 flex-col gap-4 xl:w-[300px]">
                  ...
                  >
                    ...
                    <WorkflowProgressDonut steps={stepProgress} total={admissions.length} />
                  </div>
                  ...
                  >
                    ...
                    <div className="mt-3 flex flex-col gap-3">
                      {recentCompleted.length === 0 ? (
                        <p style={{ fontSize: 14, color: '#8A98A3' }}>
                          No admissions completed yet.
                        </p>
                      ) : (
                        recentCompleted.slice(0, 3).map((a) => (
                          <div key={a.id} className="flex items-center gap-2.5">
                            <div
                              className="font-display flex size-9 shrink-0 items-center justify-center rounded-full font-semibold text-white"
                              style={{ background: avatarColorFor(a.patientName), fontSize: 14 }}
                            >
                              {initialsOf(a.patientName)}
                            </div>
                      ...
                      <PermissionGate permission={PERMISSIONS.ENCOUNTERS_WRITE}>
                        ...
                        >
                          <span className="flex items-center gap-2.5">
                            <Plus style={{ width: 17, height: 17, color: '#00B4D8' }} />
                            New Admission
                          </span>
      ...
      {showNewAdmission && (
        <NewAdmissionModal
          ...
          onConfirm={handleCreateAdmission}
        />
      )}

      {showChecklist && <AdmissionChecklistModal onClose={() => setShowChecklist(false)} />}
    </div>
  );
}
...
function WorkflowProgressDonut({
  ...
  >((acc, s) => {
    const cumulative = acc.reduce((total, seg) => total + seg.count, 0);
    ...
    <div className="mt-2 flex flex-col items-center gap-4">
      ...
      >
        <svg
          ...
          style={{ width: 130, height: 130 }}
          role="img"
          aria-label="Workflow progress donut chart"
        >
