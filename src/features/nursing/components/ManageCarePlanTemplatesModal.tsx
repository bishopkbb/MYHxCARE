...
import type { CarePlanTemplate } from '@/features/nursing/__mocks__/carePlansFixtures';

const FOCUS_RING =
  'focus-visible:ring-2 focus-visible:ring-[#00B4D8]/50 focus-visible:outline-none';

function newTemplate(): CarePlanTemplate {
  return {
    id: `tpl-${Date.now()}`,
    label: 'New Template',
    problem: '',
...
export function ManageCarePlanTemplatesModal({
  templates,
  onSave,
  onClose,
}: {
  templates: CarePlanTemplate[];
  onSave: (next: CarePlanTemplate[]) => void;
  onClose: () => void;
}) {
  const [draft, setDraft] = useState<CarePlanTemplate[]>(templates);

  function update(id: string, patch: Partial<CarePlanTemplate>) {
        ...
        >
          <div>
            ...
            <p className="mt-0.5" style={{ fontSize: 14, color: '#4A7080' }}>
              These quick templates pre-fill a new care plan&apos;s problem, goal, and
              interventions.
            </p>
          </div>
          <button
            ...
            {draft.map((tpl) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  <FormField
                    label="Template Name"
                    htmlFor={`ctpl-label-${tpl.id}`}
                    className="flex-1"
                  >
                    <input
                      id={`ctpl-label-${tpl.id}`}
                      type="text"
                      value={tpl.label}
                ...
                <div className="mt-3">
                  <FormField label="Problem" htmlFor={`ctpl-problem-${tpl.id}`}>
                    <input
                      id={`ctpl-problem-${tpl.id}`}
                      type="text"
                      value={tpl.problem}
                      onChange={(e) => update(tpl.id, { problem: e.target.value })}
                      className={`h-11 w-full rounded-[10px] bg-white px-3.5 font-sans outline-none focus:ring-2 focus:ring-[#00B4D8]/40 ${FOCUS_RING}`}
                      style={{
                        fontSize: 14,
                        border: '1px solid rgba(0,100,130,0.18)',
                ...
                <div className="mt-3">
                  <FormField label="Goal" htmlFor={`ctpl-goal-${tpl.id}`}>
                    <textarea
                      id={`ctpl-goal-${tpl.id}`}
                      value={tpl.goal}
                      onChange={(e) => update(tpl.id, { goal: e.target.value })}
                      rows={2}
