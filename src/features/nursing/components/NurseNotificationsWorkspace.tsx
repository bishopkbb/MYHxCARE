...
} from '@/features/nursing/__mocks__/nurseNotificationFixtures';
...
export function NurseNotificationsWorkspace() {
  const router = useRouter();
  const toast = useToast();
  const [notifications, setNotifications] = useState<NurseNotification[]>(NURSE_NOTIFICATIONS);
  ...
  const wardOptions = useMemo(
    () =>
      Array.from(new Set(notifications.map((n) => n.ward))).map((w) => ({ value: w, label: w })),
    [notifications],
  );
  ...
  }, [notifications, priority, ward, category, dateFrom, dateTo]);
  ...
  function handleOpen(n: NurseNotification) {
    markRead(n.id);
    router.push(CATEGORY_CFG[n.category].route);
  }
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-0.5" style={{ fontSize: 14, lineHeight: '22px', color: '#4A7080' }}>
                Stay updated on your patients, ward, and shift.
              </p>
            </div>
            <button
              type="button"
              onClick={handleMarkAllRead}
              className={`flex h-11 items-center gap-1.5 rounded-[10px] px-4 font-sans font-medium transition-colors duration-150 hover:bg-white ${FOCUS_RING}`}
                        ...
                        >
                          ...
                          <div className="w-32 shrink-0 py-3 pr-2">
                            <span
                              ...
                              style={{
                                ...
                                whiteSpace: 'nowrap',
                                color: wardCfg.color,
                                border: `1px solid ${wardCfg.border}`,
                          ...
                          <div className="flex w-28 shrink-0 items-center justify-end gap-1 py-3 pr-3">
                            <button
                              ...
                              className={`flex size-8 items-center justify-center rounded-[8px] transition-colors duration-150 hover:bg-[#E6F8FD] ${FOCUS_RING}`}
                            >
                            ...
                            <button
                              ...
                              className={`flex size-8 items-center justify-center rounded-[8px] transition-colors duration-150 hover:bg-[rgba(239,68,68,0.08)] ${FOCUS_RING}`}
                            >
