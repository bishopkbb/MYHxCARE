...
import type { PatientRecord } from '@/features/patients/__mocks__/patientFixtures';

// Opened only via "Link Patient Context" — never needed for the initial
// paint, so its code stays out of this page's main bundle until then.
const PatientContextModal = dynamic(
  () => import('./PatientContextModal').then((m) => m.PatientContextModal),
...
export function StaffChatWorkspace() {
  ...
  function insertPatientContext() {
    if (!activeConversation.patientContext) {
      // Nothing to reference yet — send straight to the picker instead of a
      // dead-end toast.
      openPatientPicker();
      return;
    }
  ...
  function handleCall() {
    toast.info(
      `Calling ${activeConversation.staffName}…`,
  ...
  function startNewConversation(staff: DirectoryStaff) {
    const id = `conv-${staff.id}`;
    ...
    setNewMenuOpen(false);
    toast.success('Conversation started', `You can now message ${staff.name}.`);
  }

  const hasActiveSearch = search.trim().length > 0;
  const messageGroups = groupMessagesByDay(activeConversation.messages);
  const directoryAvailable = getDirectoryExcluding(selfId).filter(
    (d) => !conversations.some((c) => c.id === `conv-${d.id}`),
  );
    ...
    <div className="flex flex-1 flex-col overflow-hidden">
      ...
      {patientPickerOpen && (
        <PatientContextModal
          ...
          onLink={linkPatientContext}
          onRemoveContext={removePatientContext}
        />
      )}
    </div>
  );
