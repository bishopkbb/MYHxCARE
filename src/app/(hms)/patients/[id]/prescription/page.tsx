...
// ── Patient-context derivation ────────────────────────────────────────────────
// The prescription workspace needs a compact patient shape (5 headline vitals,
// active medications, one working diagnosis) rather than the full clinical
// record — these adapt MOCK_PATIENT_DETAILS into that shape per patient, the
// same way the referral and lab-order pages read MOCK_PATIENT_DETAILS directly.

const VITAL_DISPLAY: { key: VitalKey; label: string }[] = [
  { key: 'blood-pressure', label: 'BP' },
  { key: 'pulse-rate', label: 'Pulse' },
  { key: 'temperature', label: 'Temp' },
  { key: 'spo2', label: 'SpO2' },
  { key: 'resp-rate', label: 'RR' },
];

function buildVitals(detail: PatientDetailMock): PrescriptionVital[] {
  return VITAL_DISPLAY.flatMap(({ key, label }) => {
    const reading = detail.vitalSigns.readings.find((r) => r.key === key);
    if (!reading) return [];
    return [{ label, value: reading.value, abnormal: reading.status === 'abnormal' }];
  });
}

function buildActiveMedications(detail: PatientDetailMock): ActiveMedication[] {
  return detail.medications
...
function buildDiagnosis(detail: PatientDetailMock): { condition: string; icd10?: string } {
  const active = detail.medicalHistory.pastDiagnoses.find((d) => d.status === 'Active');
  const fallback = detail.medicalHistory.pastDiagnoses[0];
  return { condition: (active ?? fallback)?.condition ?? 'No diagnosis on record' };
}
...
function buildPrescriptionPatient(id: string): PrescriptionPatient {
  ...
  return {
    ...
    vitals: buildVitals(detail),
    activeMedications: buildActiveMedications(detail),
    diagnosis: buildDiagnosis(detail),
    notes,
...
export default function PatientPrescriptionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  function updateLine(lineId: string, patch: Partial<PrescriptionLine>) {
