...
// Pill filter row — same visual family as the category filters on the
// per-patient Clinical Timeline screen, so the two screens read as one
// cohesive feature rather than two unrelated tables.
...
function SkeletonDesktopRow({ isLast }: { isLast: boolean }) {
  return (
    ...
    >
      ...
      <div className="w-20 shrink-0 py-4 pr-5">
...
export default function ClinicalTimelineIndexPage() {
  ...
  function openTimeline(patient: PatientRecord) {
    router.push(ROUTES.patientTimeline(patient.id));
  }
    ...
    <div className="px-4 pt-6 pb-24 sm:px-6 lg:px-12 lg:pt-10">
      ...
      <div>
        ...
        <p className="mt-1 text-base leading-6" style={{ color: '#4A7080' }}>
          Select a patient to view their complete chronological clinical history.
        </p>
      </div>
      ...
      <div className="mt-6 hidden overflow-hidden rounded-[12px] lg:block">
        ...
        >
          ...
          >
            ...
            <span
              className="w-20 shrink-0 py-3.5 pr-5 text-sm leading-[22px] font-bold tracking-wider uppercase"
              style={{ color: '#4A7080' }}
            >
              View
            </span>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-20 shrink-0 py-4 pr-5">
                      <button
                        type="button"
                        aria-label={`View clinical timeline for ${patient.name}`}
                        onClick={() => openTimeline(patient)}
                        className={`flex size-11 shrink-0 cursor-pointer items-center justify-center rounded-[8px] transition-opacity duration-150 hover:opacity-80 ${FOCUS_RING}`}
